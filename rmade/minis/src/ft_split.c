...
int		ft_split_count_words(char *str, char *charset)
{
	...
	while (str[c] != '\0')
	{
		if (strncmp(&str[c], charset, ft_strlenx(charset)) == 0)
		{
			c += ft_strlenx(charset);
			lword = 0;
		}
		else
		{
	...
	while (str[c] != '\0')
	{
		...
		if (((strncmp(&str[c], charset, ft_strlenx(charset)) == 0)
			|| str[c] == '\0') && word == nword)
			{
				...
				return (strncpy(strword, &str[c - (lword)], lword));
			}
	}
	return (NULL);
}
...
char **ft_split(char *str, char *charset)
{
	...
	while (i < nbword)
	{
		tab[i] = strdup(ft_split_get_word(str, charset, i + 1));
		i++;
	}
